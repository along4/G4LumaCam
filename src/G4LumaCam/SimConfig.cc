...
namespace Sim {
    ...
    void ComputePulseStructure(G4int totalNeutrons) {
        ...
        // SCINT_SIZE is the full dimension of the scintillator
        ...
        if (avgNeutronsPerPulse < 1.0) {
            ...
            // Calculate how many pulses per neutron
            G4double pulsesPerNeutron = 1.0 / avgNeutronsPerPulse;
            
            for (G4int n = 0; n < totalNeutrons; ++n) {
                // Assign each neutron to a pulse with proper spacing
                G4int pulseIndex = std::floor(n * pulsesPerNeutron);
                G4double pulseTime = pulseIndex * pulsePeriod * s / ns;
            ...
            for (G4int i = 0; i < numPulses && remainingNeutrons > 0; ++i) {
                // Poisson-like distribution around average
                ...
                pulseTimes.push_back(i * pulsePeriod * s / ns);
                
                totalAssigned += neutronsThisPulse;
                remainingNeutrons -= neutronsThisPulse;
                
                if (i < 5) {
                    G4cout << "Pulse " << i << ": t=" << (i * pulsePeriod * 1e3) 
